			...
			for target := range targetCh {
				...
				if err != nil {
					...
					if crit > 0 {
						...
						// Check if there are HNDL-EXPOSED findings
						hndl := 0
						for _, r := range results {
							if r.RiskLevel == RiskHNDL {
								hndl++
							}
						}
						if hndl > 0 {
							yellow.Printf("  [%s] %d/%d  %5.1f%%  ⚠ %s — %d quantum-vulnerable findings\n",
								bar, current, total, pct, target, hndl)
						} else {
...
func PrintAggregateReport(report AggregateReport) {
	...
	for _, tr := range report.Targets {
		...
		} else if targetVuln > 0 {
			yellow.Printf("   ⚠  %-25s %s  %5.1f%%  %d/%d vulnerable\n",
				tr.Target, miniBar, targetPct, targetVuln, targetTotal)
		} else {
	...
	if report.TotalVulnerable > 0 {
		...
		if report.TotalCritical > 0 {
			red.Println("   ALL TARGETS FAILING — Classical vulnerabilities present")
		} else {
			yellow.Println("   ALL TARGETS FAILING — Quantum migration required before 2033")
		}
		fmt.Println()
		yellow.Println(" ⚠ HARVEST NOW, DECRYPT LATER:")
		yellow.Println("   Encrypted traffic across scanned targets is at risk")
